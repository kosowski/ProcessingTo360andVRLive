...
public class OVRGazePointer : MonoBehaviour {
    ...
    public float reticleGrowthSpeed = 8;
    ...
    void Update () 
    {
        ...
        transform.position = camera.transform.position + camera.transform.forward * depth;
    ...
    public void SetPosition(Vector3 pos, Vector3 normal)
    {
        ...
        newRot.SetLookRotation(normal, camera.transform.up);
        transform.rotation = newRot;

        // record depth so that distance doesn't pop when pointer leaves an object
        depth = (camera.transform.position - pos).magnitude;
    ...
    {
        SetPosition(pos, camera.transform.forward);
    }

    /// <summary>
    /// SetPosition overload without normal and position. 
    /// Just makes cursor face user in a far position.
    /// This is used in case the raycast does not hit any object
    /// </summary>
    public void SetPosition()
    {
        SetPosition(transform.position, camera.transform.forward);
    }
    ...
    void LateUpdate()
    {
        ...
        if (positionSetsThisFrame == 0)
        {
            ...
            Quaternion newRot = transform.rotation;
            newRot.SetLookRotation(camera.transform.forward, camera.transform.up);
            transform.rotation = newRot;
        }
